...
ggsave(paste0(path.output, "SI_VISUAL_2_DAC_Projects.png"),
       bg = "white", width = 25, height = 20, units = "cm")
...
policy_case_min <- policy_case_min_aviation + policy_case_min_maritime + policy_case_min_chemicals + policy_case_min_dac_cdr
...
combined_plot <- ggplot(combined_data, aes(x = value, fill = case)) +
  geom_density(alpha = 0.5, bounds = c(base_case_min/1e3, base_case_max/1e3)) +
...
# B) Case with enhanced technology policy
...
#SI_Visual 3c: Global LNG (Parameter Distribution)
```{r, message = FALSE}
plots <- plotParameterDistributions(data.input = data.input,
                                    data.input.param = data.input.param,
                                    tech_to_plot = "LNG")  
...
#VISUAL 1: A) Base case versus B) case with enhanced technology policy and C) the individual technology policy levers 
...
# Title for the lower part “C) Individual technology policy levers”
title_lower_plots <- ggdraw() +
  draw_label(
    "C        Individual technology policy levers",  # Title for the lower plots
    fontface = 'bold', 
    size = 10,
...
ggsave(paste0(path.output, "VISUAL_1_Probabilistic_Feasibility_Space_AMMONIA.png"),
       plot = final_plot,
       width = 30, height = 32, units = "cm", bg = "white")
ggsave(paste0(path.output, "VISUAL_1_Probabilistic_Feasibility_Space_AMMONIA.pdf"),
       plot = final_plot,
       width = 30, height = 32, units = "cm", bg = "white")
```
...
#SI_Visual 7: A) Base case versus B) case with enhanced technology policy and C) the individual technology policy levers (Global)
...
#SI_Visual 8: A) Base case versus B) case with enhanced technology policy and C) the individual technology policy levers (Global)
...
#SI_Visual 9: A) Base case versus B) case with enhanced technology policy and C) the individual technology policy levers (Global)
