## Call simulation
calcTechnologyDiffusionPolicy <- function(data.demand, data.input, delta.t){
  ...
  for (i in 1:dim(data.full)[1]){
    ...
    data.temp <- data.full[i,] %>% 
      ...
      group_by(region, anticipation, sample, growth, start, dac_target_policy) %>%  # Add dac_target_policy here
      ...
      mutate(forecast = purrr::accumulate(
        demand,
        ~ {
          nxt <- .x + first(growth) * (1 - .x / .y) * .x
          nxt <- pmax(.x, pmin(nxt, .y))  # nie rückläufig, nie über Nachfrage
        },
        .init = first(start))[1:n()]) %>% 
  ...
  # Nest sensitivities
  data.results.policy <- data.full %>% 
    group_by(region, anticipation, demand, dac_target_policy) %>%  # Include dac_target_policy here
    nest() %>% 
    ungroup() %>% 
    rename(sensitivities = data)
  
  return(data.results.policy)
}
